...
BEGIN
    ...
    WHERE 
        ...
        (@CompletionStatus IS NULL OR st.CompletionStatus = @CompletionStatus)
	ORDER BY
    CASE 
...
EXEC GetStudentTaskDetailsByID; Вывод всех задач всех студентов
EXEC GetStudentTaskDetailsByID @StudentID = 2; Вывод всех задач студента с id=2
EXEC GetStudentTaskDetailsByID @TaskID = 2; по категории задания
EXEC GetStudentTaskDetailsByID @SubTaskID = 2, @CompletionStatus = N'Не приступал';
EXEC GetStudentTaskDetailsByID @CompletionStatus = N'Выполнено'; по статусу выполнения
EXEC GetStudentTaskDetailsByID @StudentTaskID = 2; конкретная задача
EXEC GetStudentTaskDetailsByID @Description = N'Демо 2025'; конкретная задача
