...
async def view_subtask(db: Session, subtask_id):
    ...
    logger.info("[SUBTASKS_CRUD] === Запуск view_subtask ===")
    logger.info(f"[SUBTASKS_CRUD] Получение задачи с id: {subtask_id}")
...
async def view_files(db: Session, subtask_id):
    ...
    params = {"subtask_id": subtask_id}
    logger.info("=== Запуск view_files ===")
    logger.info(f"Полученные файлов задачи с id: {subtask_id}")
    return general.run_query_select(
        db=db,
        query=query,
        params=params,
...
async def view_solutions_files(db: Session, subtask_id):
    ...
    params = {"subtask_id": subtask_id}
    logger.info("=== Запуск view_solutions_files ===")
    logger.info(f"Полученные файлов с решением задачи с id: {subtask_id}")
    return general.run_query_select(
        db=db,
        query=query,
        params=params,
...
async def save_subtask(db, subtask_obj, files_blocks: list):
    ...
    logger.info("[SUBTASKS_CRUD] === Запуск save_subtask ===")
    logger.info(f"[SUBTASKS_CRUD] Полученные данные подзадачи: {subtask_obj}")
    logger.info(f"[SUBTASKS_CRUD] Количество загруженных файлов с изображением задачи: {len(files_blocks)}")
    ...
    for f in files_blocks:
        logger.info(f"[SUBTASKS_CRUD] Файл: {f.filename}")
    try:
        ...
        logger.info(f"[SUBTASKS_CRUD] Добавляем строку в таблицу SubTasks с параметрами: {params_1}")
        ...
        logger.info(f"[SUBTASKS_CRUD] Обновление блоков. Исходные блоки: {subtask_obj.Blocks}")
        ...
        logger.info(f"[SUBTASKS_CRUD] Обновление подзадачи {subtask_id} с блоками: {params_2}")
        ...
        logger.exception("Ошибка при создании подзадачи")
        return {"error": str(e)}
...
async def prepare_files_data(files: List[UploadFile], file_type: str = "file") -> list:
    ...
    for idx, f in enumerate(files, start=1):
        ...
        logger.info(f"[SUBTASKS_CRUD] {file_type.capitalize()} файл {idx}: {f.filename}, content_type={f.content_type}, size={len(content)} bytes")
    return files_data
...
def log_and_validate_files(files: Optional[List], label: str):
    ...
    logger.info(f"[SUBTASKS_CRUD] Получено файлов ({label}): {count}")

    for idx, file in enumerate(files or [], start=1):
        filename = getattr(file, "filename", None)
        logger.info(f"[SUBTASKS_CRUD] Файл {idx} ({label}): {filename if filename else 'None'}")
        if not filename:
            logger.warning(f"[SUBTASKS_CRUD] Файл без имени пропущен ({label})")
...
def save_subtask_temp_record(db: Session, subtask_id: int, student_id: int, temp_solution_paths: list, temp_files_paths: list) -> int:
    ...
    logger.info(f"[SUBTASKS_CRUD] Добавляем запись о временных файлах в таблицу SubTaskTemp")
    insert_query = """
